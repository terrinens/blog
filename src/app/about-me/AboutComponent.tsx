...
export const Section = ({title, src, children, ...option}: SectionProps & { description?: string }) => {
    return (
        <div key={`${title}`} className={'my-4'}>
            ...
            <div className={'xl:ml-2 lg:ml-2 md:ml-2'}>
                {children}
            </div>
        </div>
    )
...
const TechnologyList = ({list}: { list: { name: string, src?: string }[] }) => {
    const LiElement = (data: { name: string, src?: string; }) => (
        <li className="flex items-center justify-start m-0 mx-2 my-2">{data.src
            ...
            <span className="text-gray-800 dark:text-neutral-400 ml-1.5">{data.name}</span>
        </li>
    )
