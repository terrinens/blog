...
const SliceBlock = ({props, now}: { props: [PostCardProps, PostCardProps], now?: boolean }) => {
    return (
        <div className="grid grid-cols-2 gap-3 duration-700 ease-in-out bg-white"
             data-carousel-item={now ? "active" : ""}>
    ...
    return (
        <div id="indicators-carousel" className="relative w-full" data-carousel="static">
            <div className="relative h-56 overflow-hidden rounded-lg md:h-96">
                {slicedProps.map((data, index) => (<SliceBlock key={index} props={data} now={index === 0}/>))}
            </div>
