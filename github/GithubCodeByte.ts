import {callAPI} from "./GitConfig";
import {fileIsProgrammeLanguage, programmeLanguageMatching} from "./Config";
...
export async function getGitRepositories() {
    ...
    const ifAction = (key: string, value: string) => {
        if (!extraction.hasOwnProperty(key)) {
...
class LanguagesByte {
    ...
    public append(key: string, value: number) {
        ...
        if (!this.languagesByte.hasOwnProperty(languageName)) {
...
function extractChangeCode(files: any[]) {
    ...
    const append = (key: string, patch: string, filename: string) => {
        if (!separation.hasOwnProperty(key)) {
